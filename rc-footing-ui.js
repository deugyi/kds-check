...
for(const pre of ['fs','fp']){
 const el=k=>document.getElementById(pre+'_'+k),keys=['shearBar','fyt','fck','fy','bx','by','h','cx','cy','cover','Ns','Mxs','Mys','Nu','Mxu','Myu','weightFactor','barX','barY','spacingX','spacingY',...(pre==='fs'?['qa']:['pileCount','sx','sy','diameter','pileAllow','gapFactor'])];
 function update(){
  try{
   ...
   if(pl)el('summary').innerHTML+=dl([['자동 기초 크기 / 면적',`${F(p.bx)} × ${F(p.by)} mm / ${F(p.bx*p.by/1e6,3)} m²`],['파일 배치',`${p.pileCount}본 · 행별 ${p.rowCounts.join(" / ")}본 · 최소 간격 ${p.gapFactor}D`],['파일 배치 간격',status(pl.ok)],['파일 최소 중심 간격',`${F(pl.actualMin)} mm / 필요 2.5D = ${F(pl.spacingMin)} mm · ${status(pl.spacingXOK&&pl.spacingYOK)}`],['X / Y 가장자리 중심거리',`${F(pl.edgeX)} / ${F(pl.edgeY)} mm / 필요 1.25D = ${F(pl.edgeMin)} mm · ${status(pl.edgeXOK&&pl.edgeYOK)}`],['간격을 만족하는 기초 치수',`${F(pl.minBx)} × ${F(pl.minBy)} mm 이상 · 50 mm 단위`]])+'<p class="beam-muted">D는 파일 직경입니다. 최소 치수는 중심 간격을 2.5D 이상으로 조정한 배치를 기준으로 합니다. 간격 미충족 시 지지력·전단 충족과 별개로 배치를 수정해야 합니다.</p>' ;
   ...
   if(pl&&p.autoPoints)svg+=`<text x="250" y="395" text-anchor="middle" font-size="12">최소 중심간격 ${F(pl.actualMin)} mm ≥ 2.5D</text><text x="250" y="425" text-anchor="middle" font-size="12">최소 연단거리 X/Y ${F(pl.edgeX)} / ${F(pl.edgeY)} mm ≥ 1.25D</text>`;
