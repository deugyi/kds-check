...
for(const pre of ['fs','fp']){
 ...
 function update(){
  try{
   ...
   if(pl)el('summary').innerHTML+=dl([['파일 배치 간격',status(pl.ok)],['X / Y 중심 간격',`${F(p.sx)} / ${F(p.sy)} mm / 필요 2.5D = ${F(pl.spacingMin)} mm · ${status(pl.spacingXOK&&pl.spacingYOK)}`],['X / Y 가장자리 중심거리',`${F(pl.edgeX)} / ${F(pl.edgeY)} mm / 필요 1.25D = ${F(pl.edgeMin)} mm · ${status(pl.edgeXOK&&pl.edgeYOK)}`],['간격을 만족하는 기초 치수',`${F(pl.minBx)} × ${F(pl.minBy)} mm 이상 · 50 mm 단위`]])+'<p class="beam-muted">D는 파일 직경입니다. 최소 치수는 중심 간격을 2.5D 이상으로 조정한 배치를 기준으로 합니다. 간격 미충족 시 지지력·전단 충족과 별개로 배치를 수정해야 합니다.</p>' ;
