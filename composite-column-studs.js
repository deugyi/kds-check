 ...
 * Stud strength and detailing follow 4.8.3 (steel anchors inside composite members).
 */
(function(root){
'use strict';
function calculate(o,p){
 ...
 // 4.4.3.3(2)② sums the design shear strength Qcv of each stud. Studs inside a
 // composite member are covered by 4.8.3 (4.8.1(2)); 4.8.3.1(1) gives that design
 // strength as φv·Qnv with Qnv = Fu·Asc (4.8-3), φv = 0.65, valid when concrete
 // shear breakout does not govern (4.8.3.1(2) is not evaluated here).
 const PHI_V=.65,Qnv=A*p.Fu/1000,phiQ=PHI_V*Qnv;
 ...
 check('스터드 길이 (4.8.3(2))',p.height>=5*p.diameter,`${p.height} / 최소 ${5*p.diameter} mm (전단 전용 5d)`);
 check('길이방향 중심 간격 (4.8.3.5)',p.spacing>=4*p.diameter&&p.spacing<=32*p.diameter,`${p.spacing} / ${4*p.diameter}–${32*p.diameter} mm`);
 check('도입부 내 스터드 배치',count>0,`첫 단 ${p.edge} mm · ${count}개`);
 const plan=[];let pitch=null;
 if(round){
  const r=sec.B/2-sec.t,rh=r-p.height;
  if(rh<=0)throw Error('스터드 길이가 원형 CFT 내부 반경 이상입니다.');
  pitch=2*r*Math.sin(Math.PI/p.perLevel);
 ...
 // 4.8.3.5: 4d minimum and 32d maximum centre spacing in any direction.
 check('단면방향 중심 간격 (4.8.3.5)',transverse>=4*p.diameter&&(pitch===null||pitch<=32*p.diameter),`최소 ${Number.isFinite(transverse)?transverse.toFixed(1):'해당 없음'} · 인접 ${pitch===null?'해당 없음':pitch.toFixed(1)} / ${4*p.diameter}–${32*p.diameter} mm`);
 ...
 check('측면 콘크리트 순피복 (4.8.3.5(1))',cover>=25,`${cover.toFixed(1)} / 최소 25 mm`);
 if(src)check('주철근 간섭',barClear>=0,`최소 여유 ${barClear.toFixed(1)} mm`);
 const plate=src?sec.tf:sec.t;
 check('용접 모재 두께 (4.8.1(1))',p.diameter<=2.5*plate,`${p.diameter} / 한계 ${2.5*plate} mm`);
 ...
 return {p,Pr,ratio,share,demand,Lin,levels,z,count,required,phiQ,capacity,checks,plan,transverse,cover,barClear,outside,ok:checks.every(q=>q.ok),Qnv,phi:PHI_V};
}
