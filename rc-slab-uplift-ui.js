...
function renderGeometry(p,o){
  ...
  const rows=o.directions.map(d=>{
    const label=d.dir==='l1'?'X 방향':'Y 방향';
    return `<tr><td>${label}</td><td>${fmt(d.span,0)}</td><td>${fmt(d.ln,0)}${d.floored?' <small>(0.65 l 하한)</small>':''}</td><td>${fmt(d.transverse,0)}</td><td>${fmt(d.columnWidth,0)}</td><td>${fmt(d.middleWidth,0)}</td><td class="beam-phi">${fmt(d.Mo,1)}</td></tr>`;
  }).join('');
...
function renderSections(p,o){
  ...
  get('s_sections').innerHTML=`<p class="slab-note"><b>양압력은 상향입니다.</b> 통상 중력하중 슬래브와 인장면이 반대입니다. 중앙부 정모멘트는 <b>슬래브 상부</b>가, 받침부 부모멘트는 <b>슬래브 하부</b>가 인장입니다. 배근 위치를 반대로 넣지 않도록 확인하세요.</p>`+
    ...
    `<p class="beam-muted">제안 배근은 입력한 규격으로 φMn ≥ Mu, 최소철근량, 최대간격을 모두 만족하는 가장 넓은 간격입니다. 정착·이음·단부 연장길이는 별도입니다.</p>`;
}
...
function update(){
  ...
  try{
    ...
    renderLoad(p,o);renderGeometry(p,o);renderSections(p,o);renderBasis(p,o);
  }catch(e){
    get('s_results').hidden=true;get('s_error').hidden=false;get('s_error').textContent=e.message;
    for(const id of ['s_load','s_geometry','s_sections','s_basis'])get(id).innerHTML='';
  }
}
