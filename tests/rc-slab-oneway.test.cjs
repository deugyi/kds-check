...
const base={fck:30,fy:500,h:200,L:3,cover:20,aggregate:25,bottomBar:'D13',topBar:'D13',tempBar:'D10',bottomSpacing:150,topSpacing:200,tempSpacing:250,mode:'auto',support:'simple',environment:'dry',wu:10,Mp:20,Mn:20,V:30};
...
test('one-way UDL demands use the exact three ideal support solutions',()=>{
 ...
 r=C.calculate({...base,wu:0,h:400});near(r.load.Mp,0);near(r.load.V,0);
});
test('direct demands are preserved and isolated from inactive Wu',()=>{
 const r=C.calculate({...base,mode:'direct',wu:NaN});near(r.load.Mp,20);near(r.load.Mn,20);near(r.load.V,30);
...
test('thickness reference is conditional and absolute 100mm is checked',()=>{
 ...
 const thin=C.calculate({...base,h:95,cover:10,aggregate:10});assert.equal(thin.thickness.absolute,false);assert.equal(thin.ok,false);
...
test('bad geometry and malformed numbers cannot produce a result',()=>{
 for(const p of [{h:50},{bottomSpacing:0},{cover:Infinity},{fy:700},{fck:20},{mode:'x'},{wu:-1},{aggregate:60}])assert.throws(()=>C.calculate({...base,...p}));
});
