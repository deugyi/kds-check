...
test('total count layouts preserve all piles, centroid, spacing and biaxial equilibrium',()=>{for(let n=3;n<=36;n++){const r=F.calculate({mode:'pile',autoSize:true,pileCount:n,diameter:500,gapFactor:2.5,Mxu:40,Myu:60}),v=r.piles;assert.equal(v.length,n);near(v.reduce((s,a)=>s+a.x,0),0);near(v.reduce((s,a)=>s+a.y,0),0);near(v.reduce((s,a)=>s+a.Ru,0),r.totalU);near(v.reduce((s,a)=>s+a.Ru*a.x,0),60+r.W*r.p.weightFactor*r.p.footingX/1000);near(v.reduce((s,a)=>s+a.Ru*a.y,0),40+r.W*r.p.weightFactor*r.p.footingY/1000);assert.ok(r.pileLayout.actualMin>=1250-1e-7);assert.ok(r.pileLayout.ok);}});
