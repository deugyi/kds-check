...
test('HTML and scripts initialize with the expected controls and initial results',()=>{
  ...
  assert.equal(vm.runInContext('typeof RCColumn',context),'object');assert.match(nodes.c_pm.innerHTML,/<svg/);assert.match(nodes.c_summary.innerHTML,/kN·m/);assert.equal(vm.runInContext('typeof runSteelBeam',context),'function');assert.equal(vm.runInContext('typeof runSteelCol',context),'function');
});
