...
function renderChecks(p,g,r){
  ...
  try{
    ...
    get('b_quantities').innerHTML=`<table class="beam-quantity-table"><thead><tr><th>항목</th><th>1m당 물량</th><th>단가 (만원)</th><th>금액 (만원)</th></tr></thead><tbody><tr><td>콘크리트</td><td>${fmt(q.concrete,4)} m³</td><td>${fmt(p.concretePrice/10000,2)}</td><td>${fmt((q.concreteCost)/10000,2)}</td></tr><tr><td>인장 주철근</td><td>${fmt(q.tension,5)} tonf</td><td>${fmt(p.steelPrice/10000,2)}</td><td>${fmt((q.tension*p.steelPrice)/10000,2)}</td></tr><tr><td>상부 압축철근</td><td>${fmt(q.compression,5)} tonf</td><td>${fmt(p.steelPrice/10000,2)}</td><td>${fmt((q.compression*p.steelPrice)/10000,2)}</td></tr><tr><td>표피철근</td><td>${fmt(q.skin,5)} tonf</td><td>${fmt(p.steelPrice/10000,2)}</td><td>${fmt((q.skin*p.steelPrice)/10000,2)}</td></tr><tr><td>스터럽 · 내부 다리</td><td>${fmt(q.stirrups,5)} tonf</td><td>${fmt(p.steelPrice/10000,2)}</td><td>${fmt((q.stirrups*p.steelPrice)/10000,2)}</td></tr><tr><td>철근 할증 ${fmt(p.wastePercent,1)}%</td><td>${fmt(q.steel-q.netSteel,5)} tonf</td><td>${fmt(p.steelPrice/10000,2)}</td><td>${fmt(((q.steel-q.netSteel)*p.steelPrice)/10000,2)}</td></tr><tr><th>철근 합계</th><th>${fmt(q.steel,5)} tonf</th><th>—</th><th>${fmt(q.steelCost/10000,2)}</th></tr></tbody></table><p class="beam-quantity-total">합계 ${fmt(q.totalCost/10000,2)} 만원 / m</p><p class="beam-muted">선택한 ${r.total}-${p.bar} (${r.key}) 배치 기준. 콘크리트는 철근 체적을 공제하지 않은 총체적입니다. 철근 밀도 7,850 kg/m³, 표준중력 중량환산 tonf(수치상 질량 t와 동일)를 사용합니다.</p><p class="beam-muted">스터럽 ${fmt(q.stations,3)}조/m × ${fmt(q.cutLength,1)} mm/조. 길이 ${p.stirrupCutLength===null?'자동 개산':'직접 입력'} · 반복 구간 평균 물량이며 양끝 스터럽을 중복 가산하지 않습니다. 개별 조각 1m의 발주 개수·이음·정착·굽힘 가공 상세는 별도입니다. 단가는 콘크리트 만원/m³, 철근 만원/tonf이고 금액은 재료비입니다.</p>${!r.eligible||!sk.ok?'<p class="warn">휨 또는 표피철근 조건 미달 배치의 참고 물량입니다.</p>':''}`;
  }catch(e){error('b_quantities',e);}
  return sk;
}
function renderEconomy(p,r,eco){
  if(!eco)return;
  const row=eco.rows.find(x=>x.key===r.key),best=eco.best,choice=best?current.results.find(x=>x.key===best.key):null;
  const advice=[];
  if(!row.ok)advice.push(`선택 배치: ${row.reasons.join(' / ')}. 이 조건을 먼저 해소하세요.`);
  if(best){
    if(best.key===r.key)advice.push('현재 선택 배치가 입력 조건을 충족하는 대표 배치 중 목표 철근비에 가장 가깝습니다.');
    else advice.push(`현재 규격 ${p.bar}에서 인장철근을 ${r.total}가닥 → ${best.total}가닥으로 ${best.total>r.total?'늘리면':'줄이면'} 목표에 가까워집니다. ${best.key} 배치의 φMn은 ${fmt(choice.phiMn,2)} kN·m입니다.`);
    if(p.Mu!=null&&best.ratio>.5)advice.push('소요강도를 만족하는 후보가 목표 철근비보다 높습니다. 철근을 더 줄이기보다 보 높이·폭 확대 또는 주철근 규격 변경을 비교하고, 변경 후 휨·전단·사용성을 다시 검토하세요.');
  }else if(p.Mu!=null&&eco.maxPhi<p.Mu)advice.push(`현재 단면·주철근 규격의 휨 조건 충족 배치 최대 φMn ${fmt(eco.maxPhi,2)} kN·m이 Mu ${fmt(p.Mu,2)} kN·m보다 작습니다. 단면 확대나 철근 규격 변경을 우선 검토하세요.`);
  else advice.push('현재 입력에서 추천할 수 있는 배치가 없습니다. 휨·전단·표피철근 조건을 확인하고 스터럽 간격·철근 규격·단면을 조정하세요.');
  if(Math.max(...eco.rows.map(x=>x.ratio))<.5)advice.push('최대 3단을 채워도 목표 철근비에 못 미칩니다. 더 큰 주철근 규격을 비교할 수 있습니다. 목표 철근비만 맞추기 위한 증량은 필수가 아니며, 단면 축소는 소요강도와 처짐·정착 확인 후 판단해야 합니다.');
  if(p.Mu==null)advice.push('Mu 미입력: 목표 철근비 기준 비교입니다. 철근 감량이나 단면 축소의 안전 여부는 확정할 수 없습니다. 소요휨모멘트를 입력하면 추천 후보에 반영합니다.');
  get('b_economy').innerHTML=`<p class="beam-economy-title">${best?`✓ ${best.total}-${p.bar} · ${best.key} 배치`:'경제 기준 후보 없음'}</p><dl class="beam-values"><div><dt>기준 균형철근비 ρb</dt><dd>${fmt(100*eco.rhoB,3)}%</dd></div><div><dt>목표 ρ = 0.5ρb</dt><dd>${fmt(100*eco.target,3)}%</dd></div><div><dt>선택 배치 ρ = As/(bd)</dt><dd>${fmt(100*row.rho,3)}% · ${fmt(row.ratio,3)}ρb</dd></div><div><dt>선택 d에서 목표 As</dt><dd>${fmt(eco.target*p.b*r.d,1)} mm²</dd></div></dl><ul class="beam-basis">${advice.map(t=>`<li>${t}</li>`).join('')}</ul><p class="beam-muted">사용자 지정 경제성 지표이며 최소 공사비 최적화나 KDS의 의무 철근비가 아닙니다. ρb = 0.85η(fck/fy)β₁·εcu/(εcu+fy/Es), Es = 200,000 MPa. 압축철근이 있어도 비교 지표는 단철근 직사각형 기준 ρb를 사용하고, 실제 φMn은 기존 복철근 해석값으로 확인합니다. 추천은 현재 규격의 대표 배치에 한정하며 미입력 하중·처짐·정착은 판정하지 않습니다.</p>`;
}
function select(key){
  if(!current)return;
...
function update(){
  ...
  try{
    ...
    let eco=null;try{eco=RCBeamChecks.economy(p,g,results);}catch(e){get('b_economy').textContent=e.message;}
    current={p,...out,eco};get('b_conditions').textContent=`피복 ${fmt(p.cover,0)} mm · 골재 ${fmt(p.aggregate,0)} mm`;
    if(!results.length)throw Error(out.message||'좌우대칭·상하 정렬 조건을 만족하는 배치가 없습니다.');
    get('b_results').hidden=false;
    get('b_rows').innerHTML=results.map(r=>`<tr data-key="${r.key}" class="${eco?.best?.key===r.key?'economical':''}"><td>${r.total}-${p.bar}</td><td>${r.key} <small>(${r.counts.length}단)</small></td><td>${fmt(r.As,1)}</td><td>${fmt(r.d,1)}</td><td>${fmt(r.phi,3)}</td><td class="beam-phi">${fmt(r.phiMn,2)}</td><td>${eco?fmt(r.As/(p.b*r.d)/eco.rhoB,3):'—'}</td><td>${eco?.best?.key===r.key?'<span class="beam-economy-badge">✓ 경제 기준 근접</span>':'—'}</td><td class="${r.eligible?'ok':'warn'}">${r.eligible?'충족':r.reasons.join('<br>')}</td><td><button type="button" class="beam-view" data-layout="${r.key}" aria-label="${r.key} 배치 보기" aria-pressed="false">보기</button></td></tr>`).join('');
    get('b_basis').innerHTML=`<ul class="beam-basis"><li>fck = ${fmt(p.fck,2)} MPa · fy = ${fmt(p.fy,0)} MPa · fyt = ${fmt(p.fyt,0)} MPa (전단강도에 사용)</li><li>상부 압축철근: ${g.compression?`${g.compression.count}-${p.compressionBar}, d′ = ${fmt(g.compression.d,2)} mm`:'없음'}. 실제 응력은 변형률로 계산하며 압축블록과 중복되는 철근 면적은 공제합니다.</li><li>${p.bar}: 공칭지름 ${fmt(g.bar.diameter,2)} mm, 공칭단면적 ${fmt(g.bar.area,2)} mm² · ${p.stirrup}: 공칭지름 ${fmt(g.st.diameter,2)} mm</li><li>수평 최소 순간격 = max(25, db, 4/3 × 골재) = ${fmt(g.horizontalClear,2)} mm</li><li>수직 순간격 = max(25, 4/3 × 골재) = ${fmt(g.verticalClear,2)} mm</li><li>스터럽 굽힘 내면 반지름 r = 2 × 스터럽 지름 = ${fmt(g.bend,2)} mm. 모서리 철근은 굽힘 안쪽에 물리므로 중심이 표면에서 ${fmt(g.edge,2)} mm에 놓입니다.</li><li>한 단 최대 = floor((b − 2 × 모서리 중심거리 + db + 수평 순간격) / (db + 수평 순간격)) = ${g.perLayer}가닥</li><li>수평 배치: 하부단 좌우대칭, 상부 철근은 하부 철근과 같은 연직면에 정렬. 배치가 성립하지 않는 가닥 수는 생략합니다.</li><li>각 단 εs = εcu(c − dᵢ)/c, fs = Es·εs (±fy 이내), Es = 200,000 MPa. ΣF = 0을 만족하는 c를 구하고 각 단의 모멘트를 합산합니다.</li><li>φMn ≥ 1.2Mcr와 εt ≥ εmin 확인. 최소철근 면제조건은 적용하지 않습니다. 입력 Mu는 경제 후보의 소요강도 확인에 사용합니다.</li></ul><details class="beam-settings"><summary>선택 배치의 단별 계산값 · 근거 조항</summary><ul id="b_detail_rows"></ul><p>KDS 14 20 50 : 2022, 4.2.2(1)(2) / KDS 14 20 01 : 2022, 3.1.1(2)④ / KDS 14 20 20 : 2022, 4.1.1·표 4.1-2·4.1.2(4)(5)·4.2.2(1) / KDS 14 20 10 : 2021, 4.2.3 / KDS 14 20 30 : 2021, 식 4.2-2·4.2-3</p><p>철근 공칭치수: <a href="https://www.hansco.co.kr/kr/index.php?pCode=MN7000044" target="_blank" rel="noopener">KS D 3504 제조사 규격표</a></p></details>`;
    const preferred=results.find(r=>r.key===selectedKey)||results.find(r=>r.key===eco?.best?.key)||results.filter(r=>r.eligible).reduce((a,b)=>!a||b.phiMn>a.phiMn?b:a,null)||results[0];select(preferred.key);
  }catch(e){current=null;get('b_results').hidden=true;get('b_rows').innerHTML='';get('b_diagram').innerHTML='';get('b_selected').innerHTML='';get('b_error').hidden=false;get('b_error').textContent=e.message;for(const id of ['b_skin','b_shear','b_quantities','b_economy'])get(id).innerHTML='';}
}
get('b_rows').addEventListener('click',e=>{const b=e.target.closest('button[data-layout]');if(b)select(b.dataset.layout);});
get('b_print').addEventListener('click',()=>{
  const closed=[...document.querySelectorAll('#t1 details')].filter(d=>!d.open);
