...
function read(){const n=id=>{const v=get(id).value.trim();return v===''?NaN:Number(v);};return {b:n('b_b'),h:n('b_h'),bar:get('b_bar').value,stirrup:get('b_stirrup').value,fck:n('b_fck'),fy:n('b_fy'),cover:n('b_cover'),aggregate:n('b_aggregate')};}
function diagram(p,g,r){
  ...
  return `<svg viewBox="0 0 ${w+120} ${h+90}" role="img" aria-label="${p.b}×${p.h} mm 보, ${p.bar} ${r.total}가닥, 하부부터 ${r.key} 배치"><rect x="${x}" y="${y}" width="${w}" height="${h}" fill="var(--code)" stroke="var(--ink)"/><rect x="${x}" y="${y}" width="${w}" height="${Math.min(r.a,p.h)*scale}" fill="var(--accent)" opacity=".12"/><rect x="${x+(p.cover+g.st.diameter/2)*scale}" y="${y+(p.cover+g.st.diameter/2)*scale}" width="${(p.b-2*p.cover-g.st.diameter)*scale}" height="${(p.h-2*p.cover-g.st.diameter)*scale}" fill="none" stroke="var(--dim)" stroke-width="${g.st.diameter*scale}"/>${circles}<line x1="${x-8}" x2="${x+w+8}" y1="${y+Math.min(r.c,p.h)*scale}" y2="${y+Math.min(r.c,p.h)*scale}" stroke="var(--accent)" stroke-dasharray="5 4"/><text x="${x+w/2}" y="${y+h+27}" text-anchor="middle">b = ${fmt(p.b,0)} mm</text><text x="${x+w+30}" y="${y+h/2}" text-anchor="middle" transform="rotate(90 ${x+w+30} ${y+h/2})">h = ${fmt(p.h,0)} mm</text><text x="${x}" y="18">상부 압축</text></svg>`;
}
function select(key){
  if(!current)return;
  ...
  get('b_selected').innerHTML=`<p class="beam-layout">${r.total}-${p.bar} <span>${r.key} · ${r.counts.length}단</span></p><p class="beam-capacity">${fmt(r.phiMn,2)} <small>kN·m</small></p><p>설계휨강도 φMn</p><p class="${r.eligible?'ok':'warn'}">${r.eligible?'휨 단면조건 충족':r.reasons.join(' / ')}</p><dl class="beam-values"><div><dt>총 철근량 As</dt><dd>${fmt(r.As,1)} mm²</dd></div><div><dt>도심 유효깊이 d</dt><dd>${fmt(r.d,2)} mm</dd></div><div><dt>중립축 c</dt><dd>${fmt(r.c,2)} mm</dd></div><div><dt>최외단 변형률 εt</dt><dd>${fmt(r.et,5)} / 최소 ${fmt(r.emin,4)}</dd></div><div><dt>강도감소계수 φ</dt><dd>${fmt(r.phi,4)}</dd></div><div><dt>최소철근 기준 1.2Mcr</dt><dd>${fmt(r.minMoment,2)} kN·m</dd></div></dl><p class="beam-muted">점선: 중립축 · 음영: 압축블록<br>갈고리·굽힘부를 생략한 배치도입니다.</p>`;
