...
function run(){
 ...
 try{
  ...
  $('suggestions').innerHTML=['bottom','top'].map(k=>{const r=o.suggestions[k];return `<p>${k==='bottom'?'하부':'상부'}: ${!o[k].required?'인장 소요 없음 · 제안 대상 아님':r?`${r.bar} @ ${r.spacing} mm <button type="button" class="beam-view" data-apply="${k}">간격 적용</button>`:'현재 철근 규격의 후보 없음 · 두께 또는 규격 조정 필요'}</p>`;}).join('')+'<p class="beam-muted">선택한 규격에서 휨·최소철근·연성·간격을 충족하는 가장 넓은 75–300 mm 후보 간격입니다. 전단·두께·수축철근 조건은 별도로 확인합니다.</p>';
  ...
  $('basis').innerHTML=values([['재료',`fck ${o.p.fck} / fy ${o.p.fy} MPa`],['치수·피복',`h ${o.p.h} mm / L ${o.p.L} m / 피복 ${o.p.cover} mm`],['골재·노출환경',`25 mm (내부 고정) / ${o.p.environment==='dry'?'건조':'기타'}`]])+'<p>주철근은 상·하부 각 면의 최외측, 수축·온도철근은 그 안쪽에 배치합니다. 각 휨 검토는 해당 인장면 철근만 사용하며 반대편 압축철근 효과는 산입하지 않습니다. 휨모멘트가 발생하는 면마다 최소 휨철근량을 확보합니다. 해당 부호의 모멘트가 없는 면은 휨·최소철근·주철근 간격 판정에서 제외하고 순간격만 확인합니다. 처짐을 계산하지 않으므로 KDS 14 20 30 표 4.2-1 참고 두께 미달은 종합 판정에서 미달로 봅니다. 직각방향 수축·온도철근량은 양면 합으로 검토합니다.</p><p>KDS 14 20 10 4.2.2 식 (4.2-1), (4.2-2) / 14 20 70 4.1.1 / 14 20 20 4.1.2, 4.2.2(3), 4.2.3 / 14 20 50 4.2.2, 4.6.2 / 14 20 22 4.2.1(1), 4.3.3(1) / 14 20 30 표 4.2-1.</p><p>일정 두께·축력 없는 일반 RC 슬래브입니다. 연속 슬래브의 모멘트 수정·활하중 패턴, 개구부, 뚫림전단, 정착·이음·단부 상세, 피복의 노출등급 적합성은 별도 검토합니다. 연속단은 회전을 완전히 구속하는 이상화입니다. 1단 연속은 왼쪽 단순·오른쪽 회전구속으로 해석합니다. 인접 경간 강성과 재하 패턴을 포함한 연속 슬래브 전체 해석을 대신하지 않습니다. 자동 하중조합은 D와 L만 작용하는 중력하중 범위이며 다른 하중은 포함하지 않습니다.</p>';
