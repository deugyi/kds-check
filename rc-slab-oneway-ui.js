...
const nums=['fck','fy','h','L','cover','dead','live','point','pointX','Mp','Mn','V'];
const keys=[...nums,'mode','support','environment'];
...
function loadGraph(o){
 ...
 const ends={simple:['pin','pin'],fixed:['fixed','fixed'],propped:['pin','fixed'],proppedReverse:['fixed','pin'],cantilever:['fixed','free'],cantileverReverse:['free','fixed']}[p.support];
 ends.forEach((type,i)=>{const x=i?735:65;if(type==='pin')s+=`<path d="M${x} 100l-12 20h24z" fill="none" stroke="var(--ink)"/>`;if(type==='fixed')s+=`<rect x="${x-5}" y="80" width="10" height="40" fill="var(--dim)"/>`;});
...
function run(){
 ...
 try{
  ...
  $('flexure').innerHTML='<table class="beam-table"><thead><tr><th>위치</th><th>배근</th><th>As (mm²/m)</th><th>Mu (kN·m/m)</th><th>φMn (kN·m/m)</th><th>결과</th></tr></thead><tbody>'+[o.bottom,o.top].map(r=>`<tr><td>${r.which==='bottom'?'하부 · 정':'상부 · 부'}</td><td>${r.bar} @ ${f(r.spacing,0)}</td><td>${f(r.As,1)}</td><td>${f(r.Mu)}</td><td>${f(r.phiMn)}</td><td>${badge(r.ok)}</td></tr>`).join('')+'</tbody></table>'+[o.bottom,o.top].map(r=>`<p class="beam-muted">${r.which==='bottom'?'하부':'상부'}: ${!r.required?'해당 부호 모멘트 없음 · 휨·최소철근·간격 판정 제외, 순간격만 확인'+(r.reasons.length?' · '+r.reasons.join(' · '):''):r.reasons.length?r.reasons.join(' · '):'검토 항목 충족'} · φ=${f(r.phi,3)}, εt=${f(r.et,5)}</p>`).join('');
  $('details').innerHTML=values([['최소 휨철근량 · 모멘트가 있는 면',`${f(o.bottom.AsMin,1)} mm²/m`],['위험단면 최대간격',`${f(o.bottom.detailMax,0)} mm`],['균열제어 최대간격 · fs=2fy/3',`${f(o.bottom.crackMax,1)} mm`],['수축·온도철근 · 각 면',`${o.p.tempBar} @ ${f(o.p.tempSpacing,0)} mm`],['수축·온도철근량 · 양면 합',`${f(o.temp.As,1)} / 필요 ${f(o.temp.AsMin,1)} mm²/m ${badge(o.temp.ok)}`],['수축·온도철근 최대간격',`${f(o.temp.maxSpacing,0)} mm`],['전단 유효깊이 · 작은 d 적용',`${f(o.shear.d)} mm`],['콘크리트 설계전단강도 φVc',`${f(o.shear.phiVc)} kN/m / Vu ${f(o.shear.V)} kN/m ${badge(o.shear.ok)}`]]);
  ...
  $('suggestions').innerHTML=values([['상·하부 주철근 공통 간격',`${o.commonSpacing} mm`],['하부 주철근',`${o.bottom.bar} @ ${o.commonSpacing} mm`],['상부 주철근',`${o.top.bar} @ ${o.commonSpacing} mm`],['배력근 · 상·하부 동일',`${o.p.tempBar} @ ${o.p.tempSpacing} mm`]])+'<p class="beam-muted">D10–D25, 주철근·배력근 간격 100–300 mm (10 mm 단위)를 비교하여 조건을 만족하는 최소 철근량 조합을 자동 제안합니다. 배력근도 자동 설계합니다. 모멘트가 없는 면에도 조립을 위한 최소량을 배치하며, 상·하부 주철근 간격은 항상 같습니다. 전단·처짐 참고두께 부족은 철근 증가로 해결하지 않고 두께를 조정해야 합니다.</p>';
  ...
  $('basis').innerHTML=values([['재료',`fck ${o.p.fck} / fy ${o.p.fy} MPa`],['치수·피복',`h ${o.p.h} mm / L ${o.p.L} m / 피복 ${o.p.cover} mm`],['골재·노출환경',`25 mm (내부 고정) / ${o.p.environment==='dry'?'건조':'기타'}`]])+'<p>주철근은 상·하부 각 면의 최외측, 수축·온도철근은 그 안쪽에 배치합니다. 각 휨 검토는 해당 인장면 철근만 사용하며 반대편 압축철근 효과는 산입하지 않습니다. 휨모멘트가 발생하는 면마다 최소 휨철근량을 확보합니다. 해당 부호의 모멘트가 없는 면은 휨·최소철근·주철근 간격 판정에서 제외하고 순간격만 확인합니다. 처짐을 계산하지 않으므로 KDS 14 20 30 표 4.2-1 참고 두께 미달은 종합 판정에서 미달로 봅니다. 직각방향 수축·온도철근량은 양면 합으로 검토합니다.</p><p>KDS 14 20 10 4.2.2 식 (4.2-1), (4.2-2) / 14 20 70 4.1.1 / 14 20 20 4.1.2, 4.2.2(3), 4.2.3 / 14 20 50 4.2.2, 4.6.2 / 14 20 22 4.2.1(1), 4.3.3(1) / 14 20 30 표 4.2-1.</p><p>일정 두께·축력 없는 일반 RC 슬래브입니다. 연속 슬래브의 모멘트 수정·활하중 패턴, 개구부, 뚫림전단, 정착·이음·단부 상세, 피복의 노출등급 적합성은 별도 검토합니다. 연속단은 회전을 완전히 구속하는 이상화입니다. 혼합 지지와 캔틸레버는 고정단의 좌·우 위치를 선택할 수 있습니다. 인접 경간 강성과 재하 패턴을 포함한 연속 슬래브 전체 해석을 대신하지 않습니다. 자동 하중조합은 D와 L만 작용하는 중력하중 범위이며 다른 하중은 포함하지 않습니다.</p>';
