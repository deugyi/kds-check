...
function renderCalculations(){
  const p={b:num('tb_b'),h:num('tb_h'),span:num('tb_span'),fy:num('tb_fy'),fyt:num('tb_fyt'),fyd:num('tb_fyd'),heights,strengths,
    ...
    dowel:get('tb_dowel').value,dowelCount:num('tb_dowel_count'),dowelSpacing:num('tb_dowel_spacing'),crossLegs:num('tb_cross_legs'),crossAnchored:get('tb_cross_anchored').checked,dowelAnchored:get('tb_dowel_anchored').checked};
  const auto=get('tb_strength_mode').value==='estimate';
  ...
  get('tb_joint_results').innerHTML=joints.length?`<div class="beam-table-wrap"><table class="beam-table"><thead><tr><th>시점</th><th>접합면 · 바닥 기준</th><th>소요 전단흐름 (kN/m)</th><th>스터럽의 다월바 기여 (kN/m)</th><th>추가 Avf (mm²/m)</th><th>입력 배치 강도 (kN/m)</th><th>추가 다월바 소요량</th><th>조건 확인</th></tr></thead><tbody>${joints.join('')}</tbody></table></div><p class="beam-muted">전체 자중을 해당 시점의 합성단면에 적용한 접합면 예비 검토입니다. 전단흐름은 환산 비균열단면과 인장 콘크리트를 제외한 균열단면의 VQ/I 중 큰 값입니다. q의 N/mm는 수치상 kN/m와 같습니다. φ = 0.75, μ = 1.0, fy ≤ 500 MPa. 콘크리트 접합면 상한과 정착이 확인된 유효 스터럽 ${p.crossAnchored?p.crossLegs:0}다리의 기여를 반영합니다. 같은 철근을 추가 다월바 개수에 다시 입력하지 마세요. 표의 개수는 정착 가능한 철근량 소요값이며 정착길이·간격 상세를 승인하는 값이 아닙니다.</p>`:'<p class="beam-muted">동바리 해체 후 2개 이상 타설층의 발현강도가 입력되면 이어치기면을 검토합니다.</p>';
}
...
get('tb_legs').addEventListener('input',()=>{get('tb_cross_legs').value=String(Math.min(num('tb_cross_legs'),num('tb_legs')));update();});
