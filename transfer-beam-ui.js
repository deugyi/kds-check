...
let heights=[],strengths={},waits=[];
function estimation(){return ConcreteAge.schedule(num(get('tb_fck').value==='custom'?'tb_fck_custom':'tb_fck'),get('tb_cement').value,num('tb_temperature'),waits);}

function strengthControls(){
  const auto=get('tb_strength_mode').value==='estimate';
  get('tb_age_settings').hidden=!auto;

  const stage=get('tb_view').value==='all'?heights.length:Number(get('tb_view').value);
  if(auto){
    get('tb_strength_note').textContent=`${stage}차 검토 시점 · 이전 타설층의 누적 재령도 자동 반영합니다.`;
    try{const rows=estimation().details[stage];get('tb_strengths').innerHTML=`<div class="beam-wide"><dl class="beam-values">${rows.map((r,i)=>`<div><dt>${i+1}차 · 재령 ${fmt(r.days)}일<br>등가재령 ${fmt(r.equivalentDays)}일</dt><dd>평균강도 추정 ${fmt(r.mean)} MPa<br><b>내력 입력 ${fmt(r.applied)} MPa</b></dd></div>`).join('')}</dl><p class="beam-muted">KDS의 강도 발현식에 온도 보정 등가재령을 대입한 간편 모델입니다. 내력 입력은 추정 평균강도에서 Δf를 공제하고 28일 기준강도 이하로 제한합니다.</p></div>`;}catch(e){get('tb_strengths').textContent=e.message;}
    return;
  }
  strengths[stage]??=[];
...
function stageControls(equal=false){
  ...
  waits=Array.from({length:count},(_,i)=>waits[i]??7);
  ...
  get('tb_stages').innerHTML=heights.map((v,i)=>`<div class="tb-stage" style="--stage-color:${COLORS[i]}"><h3>${i+1}차 타설</h3><label><b>${i+1}차 타설 높이 (mm)</b><input id="tb_lift_${i}" type="number" min="1" step="50" value="${Number.isFinite(v)?v:''}"></label><label><b>${i+1}차 타설 후 경과일수 (일)</b><input id="tb_wait_${i}" type="number" min="0" step="1" value="${Number.isFinite(waits[i])?waits[i]:''}"><span>양생 후 검토하고 다음 차수를 타설할 때까지</span></label></div>`).join('');
  heights.forEach((_,i)=>get('tb_wait_'+i).addEventListener('input',()=>{waits[i]=num('tb_wait_'+i);update();}));
  strengthControls();
...
function update(){
  ...
  try{
    ...
    if(get('tb_strength_mode').value==='estimate'){estimation();strengthControls();}
    if(get('tb_strength_mode').value==='manual'&&(strengths[shown]||[]).some(v=>v!==null&&v!==undefined&&(!Number.isFinite(v)||v<0||v>90)))throw new Error('발현강도는 0–90 MPa로 입력하거나 미확인 상태로 비워 두세요.');
...
function renderCalculations(){
  const p={b:num('tb_b'),h:num('tb_h'),span:num('tb_span'),fy:num('tb_fy'),fyt:num('tb_fyt'),fyd:num('tb_fyd'),heights,strengths,
    release:num('tb_release'),deadFactor:num('tb_dead_factor'),finalVu:get('tb_final_vu').value.trim()===''?null:num('tb_final_vu'),bar:get('tb_bar').value,stirrup:get('tb_stirrup').value,
    ...
    dowel:get('tb_dowel').value,dowelCount:num('tb_dowel_count'),dowelSpacing:num('tb_dowel_spacing'),crossLegs:num('tb_legs'),crossAnchored:true,dowelAnchored:true};
  const auto=get('tb_strength_mode').value==='estimate';
  if(auto)p.strengths=estimation().strengths;
  const o=TransferBeam.calculate(p),val=v=>Number.isFinite(v)?fmt(v):'—';
  TransferBeamDiagram.render(p,o,get('tb_view').value==='all'?heights.length:Number(get('tb_view').value),COLORS);
  const rows=o.phases.map(r=>`<tr><td>${r.stage}차 ${r.wet?'타설 중':'양생 후'}${!r.wet&&r.stage===p.release?' · 동바리 해체':''}</td><td>${val(r.H)} / ${val(r.loaded)}</td><td>${val(r.fc)}</td><td>${val(r.D)}</td><td>${val(r.M)}</td><td>${val(r.V)}${r.final?`<br><small>${r.Vinput===null?'자중 · 최종 Vu 미입력':r.Vsource==='input'?'최종 입력 Vu 지배':'자중 Vu 지배'}</small>`:''}</td><td>${val(r.r?.phiMn)}</td><td>${val(r.shear?.phiVn)}</td><td>${r.message}</td></tr>`).join('');
  get('tb_phase_results').innerHTML=`<p class="beam-muted">${auto?'온도·재령 간편 추정강도 사용 · ':'확인 강도 직접 입력 · '}해체 후 양단 단순지지 · D = 24 × b × 누적 높이 (m 단위), wu = ${p.deadFactor}D, 자중 Mu = wuL²/8, 자중 Vu = wuL/2. 최종 양생 후 적용 Vu = max(자중 Vu, 입력 Vu). 타설 중 강도는 직전 양생 후 입력값을 사용하며 추가 강도 발현을 가정하지 않습니다.</p><div class="beam-table-wrap"><table class="beam-table"><thead><tr><th>시점</th><th>내력 / 하중 높이 (mm)</th><th>최소 fc (MPa)</th><th>D (kN/m)</th><th>자중 Mu (kN·m)</th><th>Vu (kN)</th><th>φMn (kN·m)</th><th>φVn (kN)</th><th>계산 상태</th></tr></thead><tbody>${rows}</tbody></table></div><p class="beam-muted">미입력 강도는 0으로 간주하지 않습니다. 동바리 지지 행의 Mu·Vu는 자중 규모를 나타내는 단순보 환산값이며 실제 보 분담력을 뜻하지 않습니다. 최종 입력 Vu는 계수 적용이 끝난 총 전단력이며 자중계수를 다시 곱하지 않습니다. Vu만으로 최종 Mu를 역산하지 않으므로 최종 하중의 휨 검토는 별도입니다.</p>`;
  ...
  get('tb_joint_results').innerHTML=joints.length?`<div class="beam-table-wrap"><table class="beam-table"><thead><tr><th>시점</th><th>접합면 · 바닥 기준</th><th>소요 전단흐름 (kN/m)</th><th>스터럽의 다월바 기여 (kN/m)</th><th>추가 Avf (mm²/m)</th><th>입력 배치 강도 (kN/m)</th><th>추가 다월바 소요량</th><th>조건 확인</th></tr></thead><tbody>${joints.join('')}</tbody></table></div><p class="beam-muted">각 시점의 적용 Vu를 해당 시점의 합성단면에 적용한 접합면 예비 검토입니다. 최종 양생 후에는 입력 Vu와 자중 Vu 중 큰 값으로 모든 접합면의 스터럽·추가 다월바 소요량을 산정합니다. 전단흐름은 환산 비균열단면과 인장 콘크리트를 제외한 균열단면의 VQ/I 중 큰 값입니다. q의 N/mm는 수치상 kN/m와 같습니다. φ = 0.75, μ = 1.0, fy ≤ 500 MPa. 콘크리트 접합면 상한과 양측 정착을 가정한 스터럽 ${p.crossAnchored?p.crossLegs:0}다리의 기여를 반영합니다. 같은 철근을 추가 다월바 개수에 다시 입력하지 마세요. 표의 개수는 정착 가능한 철근량 소요값이며 정착길이·간격 상세를 승인하는 값이 아닙니다.</p>`:'<p class="beam-muted">동바리 해체 후 2개 이상 타설층의 발현강도가 입력되면 이어치기면을 검토합니다.</p>';
}
get('t7').querySelectorAll('input,select').forEach(el=>el.addEventListener('input',update));
get('tb_strength_mode').addEventListener('input',()=>{strengthControls();update();});
