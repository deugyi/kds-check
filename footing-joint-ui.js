...
function render(refreshStrength=true){
 ...
 try{
  ...
  $('summary').innerHTML=dl([['검토 형식',{soil:'지내력 독립기초',pile:'파일 독립기초 · 접합면 예비 검토',mat:'매트기초 · 1 m 폭 해석력 입력'}[mode]],['기초 크기',`${F(r.g.bx)} × ${F(r.g.by)} × ${F(p.h)} mm`],['선택 시점',phaseName(ph)],['접합면 단면 모델','비균열 환산단면 · VQ/(Ib)'],['내력 적용 최소 강도',F(ph.fc)+' MPa'],['시점별 계산 상태',esc(ph.message)],['상하부 주철근',`X ${p.barX}@${p.spacing} / Y ${p.barY}@${p.spacing}`]]);
  ...
  $('joint_summary').innerHTML+='<p class="beam-muted">τu는 X·Y 최대응력의 포락 조합입니다. 같은 지점에서 산정한 응력장이 아니며, 정사각형의 동일 성분이면 √2배가 됩니다. 국부 응력장 해석 없이 이를 저감하지 않습니다.</p>';
  ...
  $('joints').innerHTML=jointRows.length?table(['시점','이어치기면','τX / τY (MPa)','포락 τu (MPa)','기존 철근 φτ (MPa)','순압축 φμσn (MPa)','콘크리트 상한 (MPa)','추가 Avf (mm²/m²)','추가 배근안','정착·판정'],jointRows):'<p class="beam-muted">두 층 이상 양생된 시점의 강도가 확인되면 접합면을 검토합니다.</p>';
