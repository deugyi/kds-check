...
function studDiagram(o,t){
 ...
 get('stud_info').textContent=t.count?`선택: ${selectedStud+1}번 · ${layer+1}단 / ${selLine+1}열 · 용접 위치 (${f(q.x)}, ${f(q.y)}, ${f(t.z[layer])}) mm · d ${t.p.diameter} × h ${t.p.height} mm · 1개 설계전단강도 ${f(t.phiQ)} kN. 클릭 또는 키보드 Enter로 선택합니다. 스터드 표식은 가독성을 위해 확대했으며 길이방향은 도입부만 표시합니다.`:'하중도입부에 배치 가능한 스터드가 없습니다.';
}
function renderStuds(o,t){
 lastStud={o,t};
 get('stud_results').innerHTML=`<p class="beam-layout">${t.count}개 배치 <span>· 소요 ${t.required}개 · ${t.levels}단 × ${t.p.perLevel}개</span></p><p class="${t.ok?'ok':'ng'}">도입부 스터드 설계전단강도·배치 ${t.ok?'충족':'미달 / 상세 확인 필요'}</p>`+values([['실제 기둥 길이 / 도입부 길이',f(t.p.length)+' / '+f(t.Lin)+' mm'],['검토 도입 축력 Pr',f(t.Pr)+' kN'],['FyAs / Pno',f(t.ratio,3)],['전달 소요력 |Vr′|',f(t.demand)+' kN'],['스터드 1개 공칭전단강도 Qnv',f(t.Qnv)+' kN · Fu·Asc (식 4.8-3)'],['스터드 1개 설계전단강도',f(t.phiQ)+' kN · φv = 0.65 (4.8.3.1)'],['도입부 스터드 강도 합',f(t.capacity)+' kN'],['도입부 외 검토',t.outside?(t.outside.ok?'입력 전단흐름 충족':'입력 전단흐름 미달')+' · '+f(t.outside.demand)+' / '+f(t.outside.capacity)+' kN/m':'해석 전단흐름 미입력 · 판정 보류']])+'<p class="beam-muted">스터드 1개 설계전단강도 = φv·Qnv = 0.65·Fu·Asc (KDS 14 31 80 4.8.3.1(1), 식 4.8-3). 하중도입부 전단접합의 Qcv는 설계전단강도이며(4.4.3.3(2)②), 합성부재 내부 스터드에는 4.8.3을 적용합니다(4.8.1(2)). 콘크리트 전단파괴가 지배하지 않는 경우에 한하며, 콘크리트 전단파괴(4.8.3.1(2))·인장·용접부 강도는 별도 검토합니다.</p>'+table(['배치·강도 항목','입력 / 한계','상태'],t.checks.map(c=>[c.label,c.detail,`<span class="${c.ok?'ok':'ng'}">${c.ok?'충족':'확인 필요'}</span>`]))+`<p class="beam-muted">KDS 14 31 80, 4.4.3.2의 축력 분배, Lin=min(2×단면 최소폭, L/3). 1개 단부의 도입부 안에 있는 스터드만 합산합니다. 부착·지압 저항은 중첩하지 않습니다.</p><p class="beam-muted">상세: 길이 ≥ 5d (4.8.3(2), 전단 전용), 중심간격은 어느 방향이든 4d–32d (4.8.3.5), 측면 순피복 ≥ 25 mm (4.8.3.5(1)), 직경 ≤ 2.5×용접 모재 두께 (4.8.1(1)). 도입부 외에는 입력한 전단흐름에 대해 같은 배치 간격을 가정합니다.</p>`;
 studDiagram(o,t);
}
function update(){
