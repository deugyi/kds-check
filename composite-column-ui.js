...
function renderStuds(o,t){
 lastStud={o,t};
 get('stud_results').innerHTML=`<p class="beam-layout">${t.count}개 배치 <span>· 소요 ${t.required}개 · ${t.levels}단 × ${t.p.perLevel}개</span></p><p class="${t.ok?'ok':'ng'}">도입부 스터드 검토강도·배치 ${t.ok?'충족':'미달 / 상세 확인 필요'}</p>`+values([['실제 기둥 길이 / 도입부 길이',f(t.p.length)+' / '+f(t.Lin)+' mm'],['검토 도입 축력 Pr',f(t.Pr)+' kN'],['FyAs / Pno',f(t.ratio,3)],['전달 소요력 |Vr′|',f(t.demand)+' kN'],['스터드 1개 공칭강도 Qn',f(t.Qn)+' kN · Rg=1.0, Rp=0.75'],['스터드 1개 검토강도',f(t.phiQ)+' kN · 0.65Qn (보수적 적용)'],['도입부 스터드 강도 합',f(t.capacity)+' kN'],['도입부 외 검토',t.outside?(t.outside.ok?'입력 전단흐름 충족':'입력 전단흐름 미달')+' · '+f(t.outside.demand)+' / '+f(t.outside.capacity)+' kN/m':'해석 전단흐름 미입력 · 판정 보류']])+'<p class="beam-muted">Qn=min(0.5Asc√(fckEc), 0.75AscFu). 프로그램은 Qn 전체에 0.65를 보수적으로 적용하고 4.8.3.1의 강재전단 상한도 확인합니다. 이는 식 (4.8-1)에 명시된 강도계수라는 뜻이 아닙니다. 콘크리트 파괴·용접 상세는 별도 검토합니다.</p>'+table(['배치·강도 항목','입력 / 한계','상태'],t.checks.map(c=>[c.label,c.detail,`<span class="${c.ok?'ok':'ng'}">${c.ok?'충족':'확인 필요'}</span>`]))+`<p class="beam-muted">KDS 14 31 80, 4.4.3.2의 축력 분배, Lin=min(2×단면 최소폭, L/3). 1개 단부의 도입부 안에 있는 스터드만 합산합니다. 부착·지압 저항은 중첩하지 않습니다.</p><p class="beam-muted">1개 검토강도는 4.8.3.1의 강재 전단 φQn=0.65FuAsc와 4.8.2.1의 추가 상한 min(0.5Asc√(fckEc), 0.75AscFu) 중 작은 값입니다. 콘크리트 파괴가 지배하지 않는 전단 전용 검토이며, 콘크리트 파괴·인장·용접부 강도는 별도 확인해야 합니다. 도입부 외에는 입력한 전단흐름에 대해 같은 배치 간격을 가정합니다.</p>`;
 studDiagram(o,t);
}
function update(){
