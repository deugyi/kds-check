...
function renderSummary(p,o){
  get('cb_summary').innerHTML=sectionView(p,o)+
    ...
    `<p class="${o.ok?'ok':'ng'}">${o.ok?'휨 · 전단 · 시공 중 모두 충족':'미달 항목이 있습니다'}</p>`+
...
function renderShearFlow(p,o){
  ...
  get('cb_flow').innerHTML=`<div class="beam-table-wrap"><table class="beam-table"><thead><tr><th>한계상태</th><th>수평전단력 (kN)</th><th></th></tr></thead><tbody>`+
    ...
    row(`강재 전단연결재 ΣQn (${p.studCount}개)`,h.studs,h.governs==='강재 전단연결재')+
    `</tbody></table></div>`+
    ...
    `<div><dt>완전합성 소요 개수</dt><dd class="${p.studCount>=o.requiredStuds?'ok':'warn'}">${o.requiredStuds}개 · 배치 ${p.studCount}개</dd></div>`+
    `<div><dt>스터드 상세</dt><dd class="${d.ok?'ok':'warn'}">${d.ok?'충족':d.reasons.join(' / ')}</dd></div></dl>`+
    `<p class="beam-muted">V′는 3가지 한계상태의 최솟값입니다 (식 4.5-1). Qn = 0.5·Asa√(fck·Ec) ≤ Rg·Rp·Asa·Fu, Rg = ${fmt(s.Rg,2)}, Rp = ${fmt(s.Rp,2)} (골데크 미사용, 형강 직접 용접 · 표 4.3-4). Ec = ${fmt(s.Ec,0)} MPa. 개수는 최대모멘트~모멘트 0 구간 기준이며 일정 간격 배치를 가정합니다 (4.8.2.3, 4.8.2.4).</p>`;
}
...
function renderBasis(p,o){
  get('cb_basis').innerHTML=`<ul class="beam-basis">`+
    ...
    `<li>스터드 상세는 직경 ≤ 2.5tf (4.8.1(1)), 길이 ≥ 4d (4.8.2(1)), 길이방향 간격 ≥ 6d, 최대 간격 ≤ min(슬래브 두께의 8배, 900 mm) (4.8.2.4)입니다.</li>`+
    `</ul><p class="slab-note"><b>원문 수식이 문서 변환에서 누락된 항목이 있습니다.</b> 식 (4.5-1a/b/c)와 식 (4.8-1) Qn, 4.5.2(2)의 h/tw 한계값은 KDS 원문에서 수식 객체로 빠져 나와, 표준형 <code>Qn = 0.5·Asa√(fck·Ec) ≤ Rg·Rp·Asa·Fu</code>, <code>h/tw ≤ 3.76√(E/Fy)</code>, <code>0.85fck·Ac</code>, <code>Fy·As</code>로 구현했습니다. 표 4.3-4의 Rg·Rp는 원문 그대로입니다.</p>`+
