...
function update(){
  try{
    ...
    const pending=o.axialOK&&o.hasMoments&&!o.combined;
    const status=!o.nonslender?'세장판 요소 · 압축강도 판정 보류':!rolled?'용접 단면의 판요소 분류 별도 · 휨좌굴 참고값':!o.axialOK?'휨좌굴 압축강도 미달':pending?'압축강도 충족 · 축력–휨 조합 판정 보류':o.hasMoments?(o.ok?'압축강도 · 축력–2축 휨 조합 충족':'축력–2축 휨 조합 미달'):'휨좌굴 압축강도 충족';
    get('sc_summary').innerHTML=`<p class="beam-layout">${label}</p><p class="beam-muted">${rolled?'KS 압연 H형강':'Built-up 용접 조립'} · ${get('sc_gr').value||'강도 직접입력'}</p><p class="beam-capacity">${fmt(o.phiPn,1)} <small>kN</small></p><p>설계압축강도 φcPn · φc = 0.90</p><p class="${!rolled||pending?'warn':o.ok?'ok':'ng'}">${status}</p>${values([['소요압축력 Pu',`${fmt(p.Pu,1)} kN`],['강도비 Pu / φcPn',fmt(o.ratio,3)],['지배 축',`${g.key}축 · ${g.key==='X'?'강축':'약축'}`]])}${o.nonslender?`<div class="sc-meter" role="img" aria-label="강도비 ${fmt(o.ratio*100,1)} 퍼센트"><span style="width:${Math.min(100,o.ratio*100)}%;background:var(${o.axialOK?'--accent':'--ng'})"></span></div>`:''}<p class="beam-muted">위 값은 압축 내력입니다. 최종 조합 검토는 아래 축력–2축 휨 상호작용 결과를 확인하세요.</p>`;
    get('sc_props').innerHTML=(ks?`<p class="beam-muted">${SteelSection.sectionSource(ks)} · J = ${fmt(ks.J,0)} mm⁴</p>`:'')+values([['단면적 Ag',`${fmt(s.A,1)} mm²`],['강축 단면2차모멘트 Ix',`${fmt(s.Ix,0)} mm⁴`],['약축 단면2차모멘트 Iy',`${fmt(s.Iy,0)} mm⁴`],['강축 회전반경 rx',`${fmt(s.rx)} mm`],['약축 회전반경 ry',`${fmt(s.ry)} mm`]])+'<p class="beam-muted">필릿을 제외한 치수 기반 계산값입니다. 규격표 단면성능과 차이가 있을 수 있습니다.</p>';
