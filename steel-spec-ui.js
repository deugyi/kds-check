(function(){
'use strict';
document.querySelectorAll('[data-steel-kind]').forEach(select=>select.addEventListener('change',()=>{
 const kind=select.value;
 for(const k of ['H','PIPE','BOX','L','CHANNEL'])document.getElementById('steel-spec-panel-'+k).hidden=k!==kind;
 document.querySelectorAll('[data-steel-kind]').forEach(s=>{s.value=kind;});
 document.getElementById('ss_'+kind.toLowerCase()+'_kind').focus();
}));
...
function diagram(r){
 ...
 if(r.kind==='CHANNEL'){
  const tw=r.tw*s,tf=r.tf*s,run=w-tw,slope=r.variant==='tapered'?Math.tan(5*Math.PI/180):0;
  const root=tf+slope*run/2,tip=tf-slope*run/2,rr=r.r*s;
  shape=`<path d="M${x} ${y}H${x+w}V${y+tip}L${x+tw+rr} ${y+root-slope*rr}Q${x+tw} ${y+root} ${x+tw} ${y+root+rr}V${y+h-root-rr}Q${x+tw} ${y+h-root} ${x+tw+rr} ${y+h-root+slope*rr}L${x+w} ${y+h-tip}V${y+h}H${x}Z"/>`;
 }
 if(r.kind==='H'){
...
for(const kind of ['H','PIPE','BOX','L','CHANNEL']){
 ...
 function filtered(){const q=$('search').value.trim().toLowerCase().replace(/[×xX*\s]/g,'');return rows.filter(r=>{
  ...
  return (!type||(kind==='CHANNEL'?r.variant===type:type==='equal'?r.H===r.B:r.H!==r.B))&&r.name.toLowerCase().replace(/[×xX*\s]/g,'').includes(q);
 });}
 ...
 function render(){
  ...
  $('summary').innerHTML=`<p class="beam-layout">${r.name}</p><p class="beam-capacity">${f(r.mass)} <small>kg/m · 단위중량${r.calculated?' (계산값)':''}</small></p>${r.issue?`<p class="steel-spec-issue">${r.issue}</p>`:''}<dl class="beam-values"><div><dt>단면적 A</dt><dd>${f(r.A)} cm²</dd></div><div><dt>단면 2차 모멘트 Ix</dt><dd>${f(r.Ix)} cm⁴</dd></div><div><dt>단면 2차 모멘트 Iy</dt><dd>${f(r.Iy)} cm⁴</dd></div><div><dt>단면 2차 반지름 ix · iy (계산값)</dt><dd>${f(Math.sqrt(r.Ix/r.A))} · ${f(Math.sqrt(r.Iy/r.A))} cm</dd></div>${(kind==='H'||kind==='CHANNEL')?`<div><dt>웨브 · 플랜지 두께 tw · tf</dt><dd>${r.tw} · ${r.tf} mm</dd></div><div><dt>필릿 반지름 r</dt><dd>${r.r} mm</dd></div>${kind==='CHANNEL'?`<div><dt>웨브 외면에서 도심까지 Cy</dt><dd>${r.Cy} cm</dd></div>${r.r2?`<div><dt>플랜지 끝 반지름 r₂</dt><dd>${r.r2} mm</dd></div>`:''}`:''}`:`<div><dt>두께 t</dt><dd>${r.t} mm</dd></div>`}</dl>`;
